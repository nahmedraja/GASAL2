...
// this is an experimenting kernel. You can make changes here to compare with the regular local kernel.
__global__ void gasal_microlocal_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, int32_t *score, int32_t *query_batch_end, int32_t *target_batch_end, int n_tasks) {
	int32_t i, j, k, m, l;
	int32_t e;
	int32_t maxHH = 0;//initialize the maximum score to zero
	int32_t prev_maxHH = 0;
		...
		for (j = 0; j < query_batch_regs; j+=1) { //query_batch sequence in columns
			...
			for (k = 28; k >= 0; k -= 4) {
				...
				for (l = 28, m = 1; m < 9; l -= 4, m++) {
					...
					FIND_MAX(curr_hm_diff, gidx + (m-1))//the current maximum score and corresponding end position on target_batch sequence
					h[m] = curr_hm_diff;

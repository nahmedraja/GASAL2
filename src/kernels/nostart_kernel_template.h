#ifndef NOSTART_KERNEL_TEMPLATE
#define NOSTART_KERNEL_TEMPLATE

template <typename T>
__global__ void gasal_local_kernel(T ALGO, uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, int32_t *score, int32_t *query_batch_end, int32_t *target_batch_end, int n_tasks) {
	...
	int32_t p[9];
	//--------------------------------------------
    if (ALGO == SEMI_GLOBAL || ALGO == GLOBAL)
    {
